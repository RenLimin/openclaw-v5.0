...
import click
import json
from finance_engine.core.db import init_db, get_db
...
def cli(ctx, db, family_id):
    ...
    if family_id is None:
        from finance_engine.core.db.repositories import FamilyRepository
...
def account_create(ctx, code, name, acc_type, balance):
    from finance_engine.core.services.account_svc import AccountService
...
def txn_add(ctx, debit, credit, amount, txn_date, note):
    ...
    from finance_engine.core.services.txn_svc import TransactionService
    from finance_engine.core.db.repositories import AccountRepository
...
def category_create(ctx, name, cat_type):
    from finance_engine.core.db.repositories import CategoryRepository
    from finance_engine.core.services.category_engine import CategoryEngine
    fid = _fid(ctx)
...
def budget_set(ctx, category_id, category, month, amount):
    from finance_engine.core.services.budget_svc import BudgetService
    from finance_engine.core.db.repositories import CategoryRepository
...
def loan_create(ctx, name, principal, rate, term, method):
    from finance_engine.core.services.loan_svc import LoanService
...
def insurance_create(ctx, name, ins_type, premium, sum_assured, term_years, payment_years, insured_name, start_date):
    from finance_engine.core.services.insurance_svc import InsuranceService
...
def portfolio_create(ctx, name):
    from finance_engine.core.services.portfolio_svc import PortfolioService
...
def report_balance_sheet(ctx):
    from finance_engine.core.services.report_svc import ReportService
...
@click.pass_context
def rate_sync(ctx):
    from finance_engine.core.services.rate_svc import RateService
...
def export_balance_sheet(ctx, output):
    from finance_engine.core.services.export_svc import ExportService
...
def advise_health(ctx, income, expenses, age):
    from finance_engine.core.services.advise_svc import AdviseService
