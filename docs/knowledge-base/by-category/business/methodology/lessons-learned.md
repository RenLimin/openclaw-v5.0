...
## 2026-09-06: 家庭理财维度 P0 建设完成（复用骨架 + ID 冲突规避）

**背景**：按 L3 架构阶段 2 建设家庭理财维度（P0）。既有骨架 8 知识 + 1 角色（financial-advisor，3 文件）+ 2 模板。
**问题**：① 存量 13 文件全部缺 `id`/`stage`，validate 报 22 条警告（含模板 39 条）；② 角色缺 references/，未达 4 文件标准；③ 存量知识缺"适用场景/注意事项"章节；④ ADR-026 已占用 `FIN-001`~`FIN-006` 作为引擎组件 ID，知识库 ID 不能复用。
**根因**：早期建设未对照 `kb_index.py` schema；骨架建设早于 ADR-026 的 FIN 组件命名；存量知识只有内容主体没有标准结构。
**修复**：① 盘点后按"补全/升级/新增"三类处理：存量 8 知识补 id（FIN-KB-001~008）+ stage + 结构章节；financial-advisor 补 references/；存量 2 模板补 id（FIN-T006/007）；② 新增 6 知识（FIN-KB-009~014：账户体系/预算/贷款核算/利率服务/债务优化/财务健康）覆盖 Rex 10 主题；③ 新增 2 角色（financial-planner/insurance-advisor，各 4 文件）达成 3 角色；④ 新增 5 模板（FIN-T001~005）达成 7 模板；⑤ 升级 README 为规范索引（FIN-000）；⑥ 全量 validate finance 维度零警告。
**关键教训（ID 命名冲突）**：业务知识库文档 ID 必须避开已存在的组件/模块 ID 前缀——建设前先 `grep -rn` 全库检查 ID 占用（含 ADR/设计文档），再定前缀规则（本维度用 `FIN-KB-*`/`FIN-R*`/`FIN-T*`）。
**预防**：① 新建维度先检查架构文档/ADR 中的组件 ID 命名，避免冲突；② 角色/模板/知识用统一的 `维度前缀-类型-序号` 命名并登记进 README 索引；③ 通用文案填充时注意占位符（如"该子领域"）必须逐篇替换为主题化内容，否则污染知识质量；④ 存量文档补结构章节时，"适用场景/注意事项"应插在内容主体与参考框架之间，且每篇必须具体化。
