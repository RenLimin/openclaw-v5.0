...
| **原生优先** | 运行时原生已有的能力(并发控制/限流/队列/嵌套深度/会话管理),L2-L4 一律复用,不重复造轮子;L2 只做原生之上的编排逻辑(任务依赖排序/错峰发起/状态跟踪),不做底层并发控制 |
...
> 适用：**所有 L2/L3/L4 新功能、新组件、新机制设计**（不限于会话编排）。
> 背景：每次新功能设计时，容易重复造轮子、与运行时原生能力冲突，导致两层控制打架、系统不稳定。本原则统一判定标准，避免每次重新讨论。
> 详细落地手册：`docs/conventions/dev-standards.md`（三查 + 六问，所有开发设计强制）。

#### 1.3.1 核心判定：做还是不做

| 场景 | 做法 | 依据 |
|---|---|---|
| 运行时原生已有的能力(并发/限流/队列/嵌套深度/会话生命周期等) | **复用，不自己做** | 原生优先，避免两层控制冲突 |
| 原生能力之上的编排/增量逻辑 | **自建，放合适层** | 原生只提供底层原语，上层业务逻辑需要自建 |
| 不同运行时行为不一致的能力 | **L1 适配层统一抽象，L2-L4 只依赖抽象** | 运行时抽象原则(ADR-012) |
| 运行时完全没有的能力 | **评估后自建** | 最小变更，按需建设 |

#### 1.3.2 新功能设计强制 Checklist（每设计必过）

每次设计新 L2/L3/L4 组件前，按以下顺序核查：

1. **查运行时原生能力** — 官方文档里有没有？有就复用，不自己做
2. **查现有 L2 资产** — 资产清单里有没有类似组件？有就复用/扩展，不新建
3. **查现有 L3/L4 资产** — 通用业务层有没有？有就继承，不重复实现
4. **确认职责边界** — 新建组件只做原生之上的增量逻辑，不做底层控制
5. **确认分层位置** — 按职责放对应层，不越级、不跨层
6. **确认可移植性** — L2-L4 不绑定具体运行时，核心逻辑与适配层分离

> 新组件设计时必须遵守本 Checklist，否则不进入建设。详细能力归属表见 `docs/conventions/dev-standards.md`。
...
| **系统备份** | 每日自动 Git 备份 + 手动备份 + 验证/恢复 | 已上线 | ✅ |
...
9. **会话生命周期管理** (2026-08-24, 08-26 状态更新)
   - 组件 ID: `pruneAfter=48h` + `deleteAfterRun` + 分级策略 (cron 已重建，每日 02:00 执行)
...
**当前**: 📐 设计中 — [L3 架构设计](./02-generic-business-layer.md) v1.4 + 理财框架 DESIGN.md v1.0 已提交，ADR-026 accepted，待启动建设
...
| L4 Bangcle PPT 模板系统(VI规范,ADR-017) | 已上线 | ✅ |
...
| 项 | 状态 | 标记 |
|---|---|---|
| L3 按业务维度逐步建设 | 家庭理财 FIN 已上线（6 引擎迁 L3 + fin-l4 实例，ADR-026/027） | ✅ |
| L4 交付中心运营引擎(BDMS) | **已上线** | ✅ |
| L4 Bangcle PPT 模板系统 | **已上线** | ✅ |
| L4 销售合同审批模块(SCA-001) | **已上线** | ✅ |
