...
| 字段 | 值 |
|---|---|
| 文档版本 | 2.13 (2026-09-03 — OCR-001 v2签名/印章检测 + SCA-001审核精度优化) |
| 文档状态 | active |
...
| **文档数字化(OCR)** | 扫描件/图片→高精度文本+签名/印章自动检测截图（多引擎+预处理+版面分析+纠错） | 已上线 | ✅ |
...
| 文档数字化(OCR) | 023 | `components/ocr-digitalization/` | `contract_ocr_v5.py`(RapidOCR主+PyMuPDF渲染+签名/印章检测+8版本预处理+40+规则纠错) | v2:签名/印章智能检测+坐标系统一+debug可视化+10页合同实测 |
...
| 2026-09-03 | 2.13 | OCR-001 v2签名/印章检测升级：PyMuPDF替换sips（DPI与像素严格对应，解决坐标偏移）+ 左右分栏联检 + OCR文字锚定签名区 + debug可视化红框 + 墨水验证分级告警。SCA-001审核精度优化：scope字段限定审核范围（P0误报从11项→1项）+ 签署日期多源智能提取。 |
