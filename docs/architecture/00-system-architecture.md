...
| 字段 | 值 |
|---|---|
| 文档版本 | 1.3 (2026-08-24 — v4.0 对比优化：上下文溢出防护状态机 + 开发状态六态标记 + 统一 Error Contract ADR-011) |
| 文档状态 | active |
...
> 本文档所有「状态」字段使用以下六态标记，避免"✅ 已上线"与"🚧 部分就绪"混淆。
...
**回写规则**：每次开发迭代完成后，更新对应章节的状态标记 + 记录版本号/日期。
...
| **上下文管理** | auto-compaction + contextWindow 校准 + **溢出防护状态机**（session pruning ❌ 死配置） | 🚧 **2/3 层实际生效** + 状态机已定义 (EXP-20260821-003, EXP-20260824-011) |
...
4a. **上下文溢出防护状态机** (2026-08-24)
   - 定位: 横切关注点 — 在 compaction 防线之上增加**调用前预算预警**
   - 问题: 今早(08-24)压缩失效事故(四环事故链)根因之一是**无预警机制** — 上下文涨到溢出才发现
   - 五态状态机:

| 状态 | 水位 | 行为 |
|---|---|---|
| **NORMAL** | <60% | 常规 Agent Loop |
| **WARN** | 60-80% | 注入 systemEvent 预警;大工具输出摘要化;要求阶段性落盘 |
| **DIVERT** | 80-90% | 新任务强制走 `sessions_spawn` 子代理;主会话只保留控制面 |
| **HARD_LIMIT** | >90% | 熔断常规推理和大工具调用;只允许摘要/写文件/归档/恢复入口 |
| **RECOVERED** | — | 压缩/分流完成后恢复 NORMAL 或 WARN |

   - 各模型水位阈值(基于实测 contextWindow 计算):

| 模型 | ctx | 60% WARN | 80% DIVERT | 90% HARD_LIMIT |
|---|---|---|---|---|
| ark-code-latest | 224k | 134k | 179k | 201k |
| kimi-k2.7-code | 256k | 153k | 204k | 230k |
| deepseek-v4-flash | 1024k | 614k | 819k | 921k |
| LongCat-2.0 | 1024k | 614k | 819k | 921k |
| glm-5.3 | 1024k | 614k | 819k | 921k |
| minimax-m3 | 1024k | 614k | 819k | 921k |

   - 当前主会话实测: **166k / 1049k (16%)** — NORMAL 状态
   - 与 compaction 的关系: 状态机是**上游预警**,compaction 是**下游压缩** — 两者互补不重叠
   - 溢出恢复流程(5 步):
     1. 冻结当前会话 — 停止追加大上下文
     2. 抽取最小状态 — 目标/已完成/未完成/关键文件/验证结果
     3. 落盘 checkpoint — 写入 `memory/YYYY-MM-DD.md` 或项目 `reports/`
     4. 新会话/子代理恢复 — 只携带 checkpoint,不携带完整历史
     5. 清理上下文源头 — 大工具输出改 artifact,长任务拆阶段
   - 验收标准: 任意单次模型调用的 `input_tokens` 不应超过该模型 contextWindow 的 80%;超过 90% 必须触发硬熔断
   - 监控: heartbeat 轮询时检查主会话 ctx 水位,达到 WARN 即通知 Rex
...
**采纳方式**: 新错误按契约记录,旧 EXP 保持原样(通过 `相关决策` 链接)
...
- 🚧 上下文管理（auto-compaction + contextWindow 实测 + **溢出防护状态机**；session pruning 层不生效 — 2/3 层实际生效）
- ✅ 配置管理（快照 + 四步流程 + 漂移检测）
- ✅ 工具策略治理（三态模型 + 六项审计）
- ✅ 记忆语义检索（本地 GGUF embedding + 健康监控）
- ✅ 知识库工具链（`kb_index.py`：六项子能力全备）
- ✅ 沙箱隔离（colima + docker + non-main，实测 uid=1000）
...
| 2026-08-24 | 1.2 | 沙箱隔离方案 B 落地：colima+docker+non-main，实测 uid=1000/写拦截/断网/WeCom 正常 |
| 2026-08-24 | 1.3 | v4.0 对比优化：① §3.2 补「上下文溢出防护状态机」(NORMAL→WARN→DIVERT→HARD_LIMIT→RECOVERED 五态 + 各模型水位阈值表 + 溢出恢复 5 步流程)；② §1.1 补「开发状态六态标记」(📐📋🚧🔨🧪✅ + 回写规则)，§6 演进路线全部改用六态；③ ADR-011 统一错误契约 (Error Contract)，§5.5 引用 + §7 ADR 清单补入 |
