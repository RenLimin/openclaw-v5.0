...
**任何宣称"完成"的长任务，交付前必须跑完整 7 步验收。**
完整规范：`docs/conventions/delivery-acceptance.md`

来源：BDMS 教训 —— 自我汇报"全部完成"，独立审计却挖出 3 个真实缺陷（has_data 忽略月份 / 模块2 缺 service / CLI 崩溃）。**自己验证自己的产出天然有结构盲点。**
...
**验收通过 = 7 项同时满足**（含凭据扫描 + Git 工作树干净）。

**四条铁律**：
- 参数被解析 ≠ 参数生效 —— 端到端跑一遍看行为
- 收参数必须用参数 —— 签名接了参数却在判定里忽略，是最隐蔽的 bug
- 修 bug 要 grep 全调用点 —— 不能只修报错的那个入口
- 验证覆盖率比验证结论更值得怀疑 —— "N 个都测了"先问覆盖了哪 N 个
