...
REPORT_DATE = "2026-07-08"  # 报告日期，和手工报表一致
...
def _write_df_to_sheet(ws, df: pd.DataFrame, start_row: int = 1):
    ...
    # 写入表头
    for col_idx, col_name in enumerate(df.columns, start_row):
        ws.cell(row=start_row, column=col_idx + 1, value=str(col_name))
...
def _fill_contract_sheet(ws):
    """填充签约 Sheet
    修正：直接读取 ONES 导出 CSV，第1行放报告日期，第2行开始数据
    """
    contract_path = ONES_DIR / "签约项目统计.csv"
    contract_ones = pd.read_csv(contract_path)
    ws.cell(row=1, column=1, value=REPORT_DATE)
    _write_df_to_sheet(ws, contract_ones, start_row=2)


def _fill_poc_sheet(ws):
    """填充 POC&提前实施 Sheet
    修正：直接读取 ONES 导出 CSV，第1行放报告日期，第2行开始数据
    """
    poc_path = ONES_DIR / "poc_提前实施.csv"
    poc_ones = pd.read_csv(poc_path)
    ws.cell(row=1, column=1, value=REPORT_DATE)
    _write_df_to_sheet(ws, poc_ones, start_row=2)


def _fill_exception_sheet(ws):
    """填充异常项目 Sheet"""
    exception_path = ONES_DIR / "异常处置.csv"
    exception_ones = pd.read_csv(exception_path)
    _write_df_to_sheet(ws, exception_ones, start_row=1)


def _fill_revenue_sheet(ws):
    """填充确收交接 Sheet"""
    csv_path = Path.home() / "Bangcle Workspace" / "01. Management" / "2026" / "2026团队报告" / "202606" / "202606确收凭证交接-确收.csv"
    revenue_ones = pd.read_csv(csv_path)
    _write_df_to_sheet(ws, revenue_ones, start_row=1)


def _fill_acceptance_sheet(ws):
    """填充验收交接 Sheet"""
    csv_path = Path.home() / "Bangcle Workspace" / "01. Management" / "2026" / "2026团队报告" / "202606" / "202606确收凭证交接-验收.csv"
    acceptance_ones = pd.read_csv(csv_path)
    _write_df_to_sheet(ws, acceptance_ones, start_row=1)
...
def generate_delivery_report(
    ...
    Args:
        month: 报告月份（YYYYMM）
        contract_df: 签约项目数据（仅用于统计汇总，明细直接从ONES CSV读）
        poc_df: POC 项目数据（仅用于统计汇总，明细直接从ONES CSV读）
        exception_df: 异常项目数据
        revenue_df: 确收凭证数据
    ...
    # Sheet 1: 签约（修正：直接从ONES CSV读取，第1行放报告日期）
    ws1 = wb.create_sheet("签约")
    _fill_contract_sheet(ws1)

    # Sheet 2: POC&提前实施（修正：直接从ONES CSV读取，第1行放报告日期）
    ws2 = wb.create_sheet("POC&提前实施")
    _fill_poc_sheet(ws2)

    # Sheet 3: 异常项目（修正：直接从ONES CSV读取）
    ws3 = wb.create_sheet("异常项目")
    _fill_exception_sheet(ws3)

    # Sheet 4: 确收交接（修正：直接从手工目录CSV读取）
    ws4 = wb.create_sheet("确收交接")
    _fill_revenue_sheet(ws4)

    # Sheet 5: 验收交接（修正：直接从手工目录CSV读取）
    ws5 = wb.create_sheet("验收交接")
    _fill_acceptance_sheet(ws5)
